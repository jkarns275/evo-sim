...
int main(int argc, char** argv) {
  ...
  spdlog::info("-------------------------------------------");
  {
    ...
    auto csv_logger = spdlog::basic_logger_st(
        "repro_flat_fitness", "results/repro_flat_fitness.csv", true);
    ...
    for (int i = 0; i < 5000; i++) {
      Simulation<SDBGenomeNoCOConfig<2>, 2> s(
          POP_SIZE,
          10,
          100,
          false,
  ...
  spdlog::info("-------------------------------------------");
  spdlog::info(
      "Reproducibility Experiments: Wall-clock time Evaluation Sequence ");
  spdlog::info("-------------------------------------------");
  {
    const int N = 2;
    double A = 100.0;
    ...
    auto csv_logger = spdlog::basic_logger_st(
        "repro_wall_clock_time_seq", "results/repro_wall_clock_seq.csv", true);
    csv_logger->set_pattern("%v");
  ...
  {
    auto csv_logger = spdlog::basic_logger_st(
        "repro_results_a_eq_b", "results/repro_a_eq_b.csv", true);
    csv_logger->set_pattern("%v");
  ...
  {
    auto csv_logger = spdlog::basic_logger_st(
        "repro_results_b_gt_a", "results/repro_b_gt_a.csv", true);
    csv_logger->set_pattern("%v");
    csv_logger->info("Experiment Name,Converged Percentage,95% CI");
    ...
    // Genomes are initialized at 0^N at the start of the search.
    auto factory = [=](Rng& rng, auto& time_value) {
      return Genome<N>(rng, time_value, std::normal_distribution<double>{0, 1});
    };
  ...
  {
    auto csv_logger = spdlog::basic_logger_st(
        "repro_results_a_eq_b_no_co", "results/repro_a_eq_b_no_co.csv", true);
    csv_logger->set_pattern("%v");
    csv_logger->info("Experiment Name,Converged Percentage,95% CI");
    ...
    for (int i = 0; i < time_functions.size(); i++) {
      ...
      auto [prop, ci] =
          run_experiment<SDBGenomeNoCOConfig<N>, N>(fc, sc, *csv_logger);
      csv_logger->info("{},{:.4f},{:.4f}", time->to_string(), prop, ci);
    }
  }
  ...
  {
    auto csv_logger = spdlog::basic_logger_st(
        "repro_results_b_gt_a_no_co", "results/repro_b_gt_a_no_co.csv", true);
    csv_logger->set_pattern("%v");
    csv_logger->info("Experiment Name,Converged Percentage,95% CI");
  ...
  spdlog::info("-------------------------------------------");
  {
    auto csv_logger = spdlog::basic_logger_st("extended_c_xd",
  "extended_c_xd.csv", true); csv_logger->set_pattern("%v");
    csv_logger->info("Experiment Name,Converged Percentage,95% CI");
  ...
  ScottDeJongBasins<N>(d * A, B)));
    }

    SimulationConfig sc{POP_SIZE, NUMBER_SIMULATED_PROCESSORS, NUMBER_GENOMES,
  init_policy::Simulated{}, false};
    ...
    for (int i = 0; i < time_functions.size(); i++) {
      ...
      auto [prop, ci] = run_experiment<SDBGenomeConfig<N>, N>(fc, sc,
  *csv_logger); csv_logger->info("{},{:.4f},{:.4f}", time->to_string(), prop,
  ci);
    }
  }
  ...
  spdlog::info("-------------------------------------------");
  {
    auto csv_logger = spdlog::basic_logger_st("extended_c_neg_xd",
  "extended_c_neg_xd.csv", true); csv_logger->set_pattern("%v");
    csv_logger->info("Experiment Name,Converged Percentage,95% CI");
