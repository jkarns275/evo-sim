...
template <unsigned N> struct ExpConfig {
  int nthreads, nruns;
...
std::pair<double, double> run_experiment(ExpConfig<N> fc, SimulationConfig sc, spdlog::logger &logger) {
  ...
  auto f = [&](int t) {
    for (int i = 0; i < fc.nruns / fc.nthreads; i++) {
      Simulation<GC, N> s(sc.pop_size, sc.np, sc.ngenomes, sc.use_sweet, &fc.fitness_eval_fn, &fc.time_eval_fn,
                          fc.factory, sc.init_type);
      s.run();
